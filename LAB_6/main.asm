; Идея программы: "переопределить" (перехватить) прерывание таймера следующим образом: отрегулировать согласно заданию скорость автоповтора ввода, а затем вызвать исходный обработчик прерывания таймера, чтобы он сделал свои дела...
; Для .COM работает int 27h + программа маленькая, не нужно несколько сегментов
.model tiny                                                 

CSEG SEGMENT                                                
    assume CS:CSEG
    ; делаем отступ для заголовка .COM-файла (размер PSP - 256 байт)
    org 100h                                                
main:
    jmp init

    is_init         db 1    ; признак того, установили ли мы резидента
    cur_speed       db 1Fh  ; задаём начальную скорость автоповтора ввода (самую минимальную - 2 симв./сек.)
    cur_time        db 0    ; время в секундах

inc_input_speed proc near
    mov ah, 02h                                             ; АН = 02h — чтение времени из RTC. Возвращает время в упакованном BCD-формате: час (в регистре СН), минуту (CL), секунду (DH) и признак коррекции летнего/зимнего времени (DL = 1 — коррекция используется, DL = 0 — нет). Признаком успешной операции является флаг CF=0.
    int 1Ah                                                 ; 1Ah - прерывание BIOS для работы с таймером

    cmp dh, cur_time
    je skip_speed_change                                    ; если время (значение количества секунд в таймере компьютера) не изменилось, то ничего не делаем

    mov cur_time, dh                                        ; если время увеличилось, то заносим его (секунды) в переменную и изменяем скорость автоповтора ввода
    dec cur_speed                                           ; с уменьшением cur_speed увеличивается скорость автоповтора ввода

    ; эта конструкция закольцовывет cur_speed в диапазоне 00h-1Fh, т.е. меняется только скорость автоповтора (в пределах 2-30 симв./сек.), а пауза перед началом автоповтора остаётся минимальной (250 мс)
    cmp cur_speed, 1Fh ; когда больше 1Fh, тогда снова устанавливаем на минимальную 1Fh                                     
    jbe set_speed
    
    mov cur_speed, 1Fh

    set_speed:
        mov al, 0F3h                                            ; команда F3h отвечает за параметры режима автоповтора нажатой клавиши ; '0' в начале числа можно и не писать, но тогда MASM будет воспринимать это как метку, потому что она начинается с буквы
        ; ОТПРАВКА КОМАНДЫ
        out 60h, al                                             ; порт 60h предназначен для работы с клавиатурой и обычно принимает пары байтов последовательно: первый - код команды, второй - данные
        ; устанавливаем скорость автоповтора ввода
        mov al, cur_speed
        ; ОТПРАВКА ДАННЫХ
        out 60h, al                                             

    skip_speed_change:
        ; переход к старому обработчику прерывания
        db 0EAh ; байт-код прямого перехода
        handler_addr dd 0 ; адрес обработчика прерываний, который мы будем перезаписывать (метка старого обработчика прерывания)
                         
inc_input_speed endp

init:
    ; AH = 35h, AL = 1Ch - получаем адрес обработчика прерывания таймера (1Ch) в ES:BX (в ES - сегментная часть адреса, в BX - смещение)
    mov ax, 351Ch
    int 21h                                                 
    ; если программа запускается не в первый раз, т.е. если адрес обработчика уже перезаписан, то выходим
    ; мы только что записали в es адрес обработчика прерывания
    cmp es:is_init, 1  
    je exit                                                 

    mov word ptr handler_addr, bx                           ; сохраняем адрес обработчика прерываний по частям в одну переменную
    mov word ptr handler_addr[2], es                        ; конструкция 'word ptr' нам нужна для того, чтобы обратиться по адресу (handler_addr + 2) как к ДВУХБАЙТОВОЙ сущности, в то время как переменная сама по себе весит 4 байта
    ; AH = 25h, AL = 1Ch - заменяем вектор 1Ch в таблице векторов прерываний на свой (адрес берётся из DX или DS:DX в зависимости от расстояния до процедуры: near или far)
    mov ax, 251Ch
    mov dx, offset inc_input_speed
    int 21h                                                 

    mov dx, offset init_msg
    mov ah, 09h
    int 21h                                                 
    ; завершаем программу резидентной ; всё, начиная с адреса метки init, будет освобождено из памяти
    mov dx, offset init
    int 27h                                                 

exit:
    mov dx, offset exit_msg 
    mov ah, 09h
    int 21h

    ; для установки характеристик режима автоповтора в порт 60h необходимо записать код команды 0F3h, затем байт, определяющий характеристики режима
    mov al, 0F3h        ; команда F3h отвечает за параметры режима автоповтора нажатой клавиши ; '0' в начале числа можно и не писать, но тогда MASM будет воспринимать это как метку, потому что она начинается с буквы
    out 60h, al         ; порт 60h предназначен для работы с клавиатурой и обычно принимает пары байтов последовательно: первый - код команды, второй - данные
    ; устанавливаем период автоповтора 30.0, задержку включения режима автоповтора 250 мс (восстанавливаем дефолтные значения)
    mov al, 0
    out 60h, al                                             
    ; восстанавливаем старый обработчик прерывания таймера в таблице векторов прерываний
    mov dx, word ptr es:handler_addr                       
    mov ds, word ptr es:handler_addr[2]
    mov ax, 251ch
    int 21h                                                 
    ; Надо ли вычищать память из-под резидента? И работает ли это корректно?
    mov ah, 49h                                             
    int 21h

    mov ax, 4c00h
    int 21h                    
	; объявляем здесь, а не вверху, чтобы эти строки не остались в резидентной части программы (они там не нужны)
    init_msg db 'New interrupt handler installed.', '$', 10, 13     
    exit_msg db 'New interrupt handler uninstalled.', '$', 10, 13
    
CSEG ENDS
END main